...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-card border border-border rounded-xl p-6 md:p-8">
        <h2 class="text-2xl font-bold text-foreground mb-6">Edit Category</h2>
        ...
        <form action="<?= base_url('categories/update/'.$category['id']) ?>" method="post">
            <div class="mb-6">
                ...
                <input type="text" name="name" id="name" class="w-full bg-muted border border-border rounded-lg py-3 px-4 text-foreground placeholder-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all" placeholder="Enter category name" value="<?= old('name', $category['name']) ?>" required>
            </div>
